...
public class MapTool extends CruderConstant {
    ...
    public static <T> T fromMap(Map<String, Object> map, Class<T> clazz) throws IllegalAccessException {
	T instance=null;
	try {
		...
		if (map.containsKey(field.getName())) {
			...
			field.set(instance, l);
		    else if (value instanceof Float l)
			field.set(instance, l);
		    else if (value instanceof Map l)
			field.set(instance, l);
    ...
    public static Map<String, Object> vo(Map<String, Object> mapData) {
	return mapObject(mapData, VO);
    }
	...
	for (int i = 1; i < dataList.size(); i++) {
		...
		for (String ki : mapData.keySet()) {
		    ...
		    if (object != null && obj != null && !object.equals(obj) || object == null && obj != null
    ...
    // public static JSONObject compatiblity(JSONObject mapData, Map<String, Object>
    // po){
	...
	try {
	    userOTP = t.getDeclaredConstructor().newInstance();
	    for (Map.Entry<String, Object> entry : map.entrySet()) {
		    String fieldName = entry.getKey();
		    Object value = entry.getValue();
		    System.out.println(fieldName+":"+value);
		    try {
			java.lang.reflect.Field field = userOTP.getClass().getDeclaredField(fieldName);
			if(field!=null) {
			    field.setAccessible(true);
...
//	objectMapper.convertValue(map, t);
